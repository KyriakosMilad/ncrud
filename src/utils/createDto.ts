...
export default async (name: string, action: string): Promise<void> => {
	try {
		...
		const content: string = dtoTemplate(name, action, Model, Models, model, models);

		const srcDir = getValueFromConfig('srcDir');
		const dir: string = `${srcDir}/${models}`;
		const filename: string = `${dir}/${action + Model}.dto.ts`;
		...
		errorHandler(`error happened cannot create schemaFile, ${err}`, '005');
		process.exit();
	}
};
